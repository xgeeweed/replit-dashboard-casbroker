...
export default function AdminLoadboard() {
  ...
  return (
    <div className="font-light h-full w-full flex flex-col p-4">
      <Tabs defaultValue="all" className="w-full">
        ...
        <div className="mb-4 mt-4 grid grid-cols-7 gap-4">
          <Input 
            placeholder="Origin"
        ...
        <TabsContent value="active">
          <DataTable 
            columns={columns} 
            data={data.filter((item: LoadboardEntry) => item.status === "Active")} 
            meta={meta}
          />
        </TabsContent>
        <TabsContent value="completed">
          <DataTable 
            columns={columns} 
            data={data.filter((item: LoadboardEntry) => item.status === "Completed")} 
            meta={meta}
          />
        </TabsContent>
        <TabsContent value="cancelled">
          <DataTable 
            columns={columns} 
            data={data.filter((item: LoadboardEntry) => item.status === "Cancelled")} 
            meta={meta}
          />
        </TabsContent>
      </Tabs>
